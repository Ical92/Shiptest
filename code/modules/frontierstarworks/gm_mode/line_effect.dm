/obj/effect/gm_mode_line
	var/image/I
	var/client/cl

/obj/effect/gm_mode_line/New(client/C, atom/atom_a, atom/atom_b, linename)
	if(!C || !atom_a || !atom_b)
		stack_trace("Buildmode effect created with odd inputs")
		return
	name = linename
	abstract_move(get_turf(atom_a))
	I = image('icons/effects/beam.dmi', src, "1-full", 19.0)
...
/obj/effect/gm_mode_line/Destroy()
	if(I)
		if(istype(cl))
			cl.images -= I
			cl = null
